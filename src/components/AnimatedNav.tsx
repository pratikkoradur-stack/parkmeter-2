...
export const AnimatedNav: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const activeButton = navElement.querySelector('ul li.active button') as HTMLElement | null;

    const init = () => {
      if (!activeButton) return;
      gsap.set(activeElement, { x: getOffsetLeft(activeButton) });
      gsap.to(activeElement as any, { '--active-element-show': '1', duration: 0.2 });
    };

    // wait for fonts ready like original
    if ((document as any).fonts && (document as any).fonts.ready) {
      (document as any).fonts.ready.then(init).catch(init);
    } else {
      setTimeout(init, 50);
    }

    const buttons = Array.from(navElement.querySelectorAll('ul li button')) as HTMLElement[];
    ...
    buttons.forEach((button, index) => {
      const handler = () => {
        ...
        gsap.set(activeElement as any, { rotateY: direction === 'before' ? '180deg' : '0deg' });

        gsap.to(activeElement as any, {
          keyframes: [
            {
              ...
              onStart: () => {
                ...
                gsap.to(activeElement as any, { '--active-element-opacity': 1, duration: 0.1 });
              },
            },
            {
              ...
              duration: 0.3,
              onStart: () => {
                gsap.to(activeElement as any, { '--active-element-mask-position': '40%', duration: 0.5 });
                gsap.to(activeElement as any, { '--active-element-opacity': 0, delay: 0.45, duration: 0.25 });
              },
              onComplete: () => {
                activeElement.innerHTML = '';
                navElement.classList.remove('before', 'after');
                activeElement.removeAttribute('style');
                gsap.set(activeElement as any, { x: getOffsetLeft(button), '--active-element-show': '1' });
              },
            },
          ],
        });

        gsap.to(activeElement as any, { x, '--active-element-strike-x': '-50%', duration: 0.6, ease: 'none' });
      };
    ...
    return () => {
      ...
      if (activeElement && activeElement.parentElement) activeElement.parentElement.removeChild(activeElement);
